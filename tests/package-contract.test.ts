...
describe('out-of-tree Bundle contract', () => {
  ...
  it('mounts the terminal entries through one valid patch list', () => {
    ...
    expect(patchText).not.toContain("name: 'seektty/attachment-compat'")
    ...
    expect(names).toContain('@deepseek-ai/dsh-api-session-controller')
    expect(names).toContain('@deepseek-ai/dsh-client-file-upload')
    expect(names).not.toContain('@deepseek-ai/dsh-host-apiproxy')
    expect(names).not.toContain('@deepseek-ai/dsh-storage')
  ...
  it('keeps pack-policy helpers out of the published files and runs pack-check from check', () => {
    ...
    expect(workflow).toContain('node scripts/install-stock-dsh.mjs "$RUNNER_TEMP/dsh-global"')
    const installer = readFileSync(resolve(root, 'scripts/install-stock-dsh.mjs'), 'utf8')
    expect(installer).toContain('stockDshTarget')
    expect(installer).toContain('verifyStockDsh')
  ...
  it('keeps both READMEs on the release version and exact npm install spec', () => {
    for (const name of ['README.md', 'README.zh.md']) {
      ...
      expect(text).toContain('DeepSeek%20Harness-0.1.5--rc.1')
    ...
    expect(english).toContain('This release declares and tests only official dsh `0.1.5-rc.1`')
    expect(chinese).toContain('本版本仅声明并验证官方 dsh `0.1.5-rc.1`')
