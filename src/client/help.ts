  ...
  if (id === 'flows') {
    return ui(
      [
        ...
        '滚动与复制：完整鼠标模式用滚轮浏览内部对话。F3 或 /mouse 可切到原生模式，用终端选择；Terminal.app 按住 Fn、iTerm2 按住 Option 后拖选。复制选区默认 Ctrl+Shift+C。',
      ].join('\n'),
      [
        ...
        'Scroll and copy: full mouse mode uses the wheel on the internal transcript. F3 or /mouse switches to native mode for terminal selection; hold Fn in Terminal.app or Option in iTerm2 while dragging. Copy selection defaults to Ctrl+Shift+C.',
      ].join('\n'),
    )
  }
  return ui(
