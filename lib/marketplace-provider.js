import { n as assertCredentialFreeUrl } from "./plugin-marketplace-D5nWF8rN.js";
...
function normalizeSource(kind, source) {
	if (!SOURCE_ID.test(source.id) || RESERVED_SOURCE_IDS.has(source.id)) throw new Error(`Source Provider ${kind} 的 Source id ${JSON.stringify(source.id)} 必须是非保留的小写 kebab-case`);
...
function normalizeDiscovery(value, kind) {
	...
	if (spec === void 0) throw new Error(`Source Provider ${kind} 返回了空插件 spec`);
...
var TuiMarketplaceProviderRegistry = class extends Service {
	...
	*/
	register(provider) {
		...
		if (!PROVIDER_KIND.test(kind) || RESERVED_KINDS.has(kind)) throw new Error(`Source Provider kind ${JSON.stringify(provider.kind)} 必须是非保留的小写 kebab-case`);
		if (this.providers.has(kind)) throw new Error(`Source Provider ${kind} 已注册`);
		if (provider.sources.length === 0) throw new Error(`Source Provider ${kind} 必须至少声明一个 Source`);
	...
	*/
	async search(query, source, signal) {
		...
		if (registered === void 0) throw new Error(`Source Provider ${JSON.stringify(source.kind)} 未注册或已卸载`);
		if (!registered.sources.some((candidate) => candidate.id === source.id)) throw new Error(`Source ${JSON.stringify(source.id)} 不属于 Provider ${source.kind}`);
