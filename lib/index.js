...
import { t as TUI_STARTUP_SERVICE } from "./startup-B2y10Pj_.js";
...
function framedRule(start, end, label, width, paint$1, align = "left") {
	if (width <= 2) return paint$1("─".repeat(Math.max(1, width)));
	...
	let interior;
	if (align === "right" && safeLabel !== "") interior = `${"─".repeat(Math.max(0, interiorWidth - visibleWidth(safeLabel) - 3))} ${safeLabel} ─`;
	else {
		const lead = safeLabel === "" ? "─" : `─ ${safeLabel} `;
		interior = `${lead}${"─".repeat(Math.max(0, interiorWidth - visibleWidth(lead)))}`;
	}
	return paint$1(`${start}${truncateToWidth(interior, width - 2)}${end}`);
}
	...
	*/
	...
	render(width) {
		...
		if (this.getText() === "" && !this.isShowingAutocomplete() && editorRows.length > 0) {
			...
			editorRows[0] = padded(`${color.brand("❯")} ${cursor}${color.muted("输入消息，/ 打开命令")}`, innerWidth);
...
function modalRule(title, width, top) {
	const start = top ? "╭" : "╰";
	const end = top ? "╮" : "╯";
	if (!top || title === void 0 || width < 8) return color.brand(`${start}${"─".repeat(Math.max(0, width - 2))}${end}`);
	...
	*/
	...
	render(width) {
		...
		for (const [index, component] of this.components.entries()) {
			...
			lines.push(...this.rows[index]?.format === "image" ? rendered : rendered.map(escapeTerminalText));
		}
