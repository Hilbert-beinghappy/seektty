...
*/
function producedForClosing$1(data, seq = Number.POSITIVE_INFINITY) {
	...
	*/
	producedFiles() {
		...
		for (const key of snapshot.chat.order.toReversed()) {
			...
			const paths = producedForClosing$1(node.location.turn.data.get("deliverables"), node.data.seq);
...
const color = {
	...
	pulse: (text, frame) => {
		...
		return paint(values[(Math.floor(frame) % values.length + values.length) % values.length] ?? palettes[selectedTheme].brand, text);
	},
...
function isMouseInput(data) {
	return SGR_MOUSE.test(data) || data.length === 6 && data.startsWith("\x1B[M");
}
function wheelDirection(button) {
	if ((button & 64) === 0) return void 0;
	const axis = button & 3;
	return axis === 0 ? "up" : axis === 1 ? "down" : void 0;
}
/**
...
function mouseWheelDirection(data) {
	const sgr$1 = SGR_MOUSE.exec(data);
	if (sgr$1 !== null) return sgr$1[2] === "M" ? wheelDirection(Number(sgr$1[1])) : void 0;
...
//#endregion
//#region src/client/compat/deliverables-rc6.ts
/**
...
function chatNodeRows(node, preferences) {
	...
	if (node.kind === "tool-call") {
		...
		return data === void 0 ? [] : grouped([{
			...
			..."kind" in data.root ? {} : { pulse: "marker" }
		}]);
	}
...
var Transcript = class {
	...
	*/
	...
	render(width) {
		...
		const olderMarker = (count) => {
			...
			return color.muted(count === 0 ? `↑ 还有更早内容 · ${hint}` : `↑ ${String(count)} 行更早内容 · ${hint}`);
		};
		const lines = [];
		const anchors = [];
	...
	*/
	...
	replace(rows) {
		...
		this.syncPulseAnimation(rows.some((row) => row.pulse !== void 0));
	}
	component(row) {
		if (row.format === "markdown") return new Markdown(escapeTerminalText(row.text), 0, 0, markdownTheme);
		if (row.format === "plain") return row.pulse === void 0 ? new Text(escapeTerminalText(row.text), 0, 0) : new PulsingRow(row.text, row.pulse, () => this.pulseFrame);
