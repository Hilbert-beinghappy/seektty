import { c as ui, s as translateUiText } from "./locale-Bf-LPk7P.js";
...
function normalizeCustomTheme(value) {
	...
	let remoteSource;
	if (record.remoteSource !== void 0) {
		const remote = recordOf$1(record.remoteSource, "customThemes[].remoteSource");
...
var Transcript = class Transcript {
	...
	plainProjectionCache = /* @__PURE__ */ new WeakMap();
	focused = false;
	...
	*/
	...
	renderBlock(blockIndex, contentWidth, projected) {
		...
		let cachedBlock = block$1.linesByWidth.get(cacheKey);
		if (clockDriven && cachedBlock !== void 0) {
			const lines$1 = [...cachedBlock.lines];
			for (const span of cachedBlock.clockSpans ?? []) {
				const rendered = span.component.render(contentWidth);
				internals.componentRenders += 1;
				const projections$1 = componentSelectionLines(span.component) ?? fallbackSelectionLines(rendered, contentWidth);
				if (rendered.length !== span.length || projections$1.some((projection, index) => {
				...
				for (const [index, line] of rendered.entries()) {
					...
					lines$1[span.start + index] = this.safeRenderedLines.get(line);
				}
			}
			if (cachedBlock !== void 0) cachedBlock = {
				...cachedBlock,
				lines: lines$1
			};
		}
		...
		for (const [index, component] of block$1.components.entries()) {
			...
			let plainProjection = row === void 0 ? void 0 : this.plainProjectionCache.get(row);
			if (row?.format === "plain" && row.pulse === void 0 && row.welcome !== true && plainProjection?.width !== contentWidth) {
				plainProjection = {
					width: contentWidth,
					...plainSelectionLayout(row.text, contentWidth)
				};
			...
			const projected$1 = row?.format === "rule" || row?.format === "image" ? rendered.map(() => ({
				...
				displayStartCell: 0,
				joinerAfter: ""
			})) : row?.format === "plain" && row.pulse === void 0 && row.welcome !== true ? plainProjection.projections : componentSelectionLines(component) ?? fallbackSelectionLines(rendered, contentWidth);
	...
	rememberOwnerCopy(key, block$1) {
		if (this.ownerCopy.get(key)?.sourceProjections === block$1.projections) return;
		...
		const next = {
			sourceProjections: block$1.projections,
