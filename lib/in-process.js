...
var InProcessConnectionService = class extends Service {
	...
	get fetch() {
		...
		return { register: (route) => owner.effect(() => {
			...
			return () => {
				if (this.fetchRoutes.get(route.path) === route) this.fetchRoutes.delete(route.path);
			};
	...
	createSharedFetchHandler(channel) {
		...
		const routeFor = (method, url) => {
			...
			return route?.methods.some((candidate) => candidate === method) ? route : void 0;
		};
		return {
			requestBodyMode: (request) => routeFor(request.method, request.url)?.requestBody ?? "buffered",
			fetch: async (request) => {
				request.signal.throwIfAborted();
				const route = routeFor(request.method, new URL(request.url));
				return route === void 0 ? new Response("Not found", { status: 404 }) : route.fetch(request);
			}
		};
	}
