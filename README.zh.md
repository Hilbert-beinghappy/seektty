> **开发分支兼容性：** 当前已测 Host 是官方 `0.1.5-rc.3`。本分支包含适配改动；已发布的 `seektty@1.2.6` 在 dsh `0.1.5-rc.1` 上验证。详见[rc.2 验证记录](docs/dsh-0.1.5-rc.2-adaptation.md)和[原生 API 迁移记录](docs/dsh-0.1.5-rc.1-adaptation.md)。
...
`seektty@1.2.6` npm 包使用已审核包输入构建，详见[发布说明](docs/release-v1.2.6.md)、[验证记录](docs/release-v1.2.6-verification.md)和[ dsh 0.1.5-rc.1 适配记录](docs/dsh-0.1.5-rc.1-adaptation.md)。
...
SeekTTY `1.2.6` 适配 dsh `0.1.5-rc.1` 原生 API，同时保留 Fastfetch 风格欢迎页、终端背景融合、TextMate 高亮、对话与选择控件改进，以及 pnpm 11 安装兼容，无需迁移 Settings 或 Session。
...
完整模式使用固定高度的 alternate screen，把输入框和状态栏固定在底部，并提供应用内滚动、选区和点击。空闲时按 F3 或执行 `/mouse native` 可切到终端原生模式：SeekTTY 离开备用屏幕，把已载入的完整对话写入普通终端滚动记录，关闭鼠标报告，由终端或 tmux 接管滚动、搜索和选区。补载按顺序执行并显示进度；Esc 可停止补载但不停止会话，Ctrl+L 只刷新活动尾部。`/transcript` 打开对话查看，`/transcript replay` 明确重新回放当前可用历史。原生模式用文本行表示图片附件；已经写入的历史不会由 SeekTTY 重新着色或重排。
...
| 输入 | 操作 |
| --- | --- |
| F3 或 `/mouse toggle` | 空闲时切换完整模式与终端原生模式 |
...
| 使用终端／tmux 的滚动与选区 | 终端原生模式使用外层终端原有的滚动、搜索、copy mode 与复制方式 |
...
右键菜单独立浮在当前页面上，不进入页面返回栈。它根据鼠标下方的语义对象提供操作，不移动列表箭头或键盘焦点：会话、工作区、Profile、主题、欢迎页信息行、Fastfetch 模块、排队消息、插件、文件、后台任务、子 Agent、卡片、Agent 树、MCP、Skill、状态控件和可编辑文本都会在适用时复用现有动作。每个根菜单固定保留“复制所选文本”和“关闭”；没有选区时复制置灰。终端原生模式继续通过 F3 或 `/mouse` 切换，不放进右键菜单。
...
界面／代码主题负责选择调色板；**显色方式**、**背景呈现**、**终端背景同步（高级）**已拆成独立设置。在 `/theme` 或 `/settings seektty-appearance` 中编辑，经过 Harness revision 保护保存后立即生效。默认值和已有外观均保持不变，读取旧设置时不会自动迁移或写回。

| 独立设置 | 直接命令 | 含义 |
| --- | --- | --- |
| 显色方式 | `/theme colors auto` 或 `/theme colors rgb` | 自动检测 RGB／256／16 色，或直接输出主题 RGB，不主动量化及调整对比度 |
| 背景呈现 | `/theme fill terminal` 或 `/theme fill theme` | 沿用终端效果，或铺满主题画布、弹窗和代码底色，包括空白单元 |
| 终端背景同步（高级） | `/theme sync off` 或 `/theme sync theme` | 禁止 OSC 11，或在支持的终端尝试临时同步主题底色 |

要同时得到原色文字和主题背景，使用 `/theme colors rgb` ＋ `/theme fill theme`。主题铺底使用 SGR 背景色，**不依赖 OSC 11**，与 `/theme sync off` 可以一起使用。它不会修改终端配置；实际颜色和透明效果仍由终端或 tmux 决定。沿用终端背景时需要手动匹配明暗主题；`NO_COLOR`、`TERM=dumb` 和非交互边界仍优先。选区、特殊 token 背景和 Logo 像素保留显式颜色。切换显色或铺底会刷新已有消息、代码和欢迎页，不重建 Session，不清空输入、选区、复制内容或滚动位置。

旧 `/theme background <组合>` 命令继续兼容，会把**三个设置一起恢复到旧组合**；`/theme background explicit` 仍可直接恢复兼容组合。无参数的 `/theme background` 现在打开背景呈现编辑器；三个新命令无参数时分别打开各自编辑器。旧设置在没有独立覆盖时按以下表格解释：
...
| `foreground`（主题原色，试验，手动启用） | 终端默认背景（`SGR 49`），保留主题 RGB 前景色 | 不查询、不改色；只恢复本次运行曾修改并捕获的原背景 |

旧 `foreground` 组合等于“原色 RGB＋沿用终端背景＋关闭同步”。在它的基础上输入 `/theme fill theme`，只增加主题铺底，保留已经奏效的 RGB 显色。不自动降级，也不保证所有终端的最终颜色一致。
...
只有“自动检测＋沿用终端背景”会适配已知且与主题不同的背景；背景未知时使用终端默认前景色，不猜黑白。“原色 RGB”绕过此处理。主题菜单在保存或从子菜单返回后，会刷新当前调色板与独立设置摘要。显色、铺底及同步属于 Harness 设置，不进入可移植主题文件。
...
当前已测 Host 是官方 `0.1.5-rc.3`。[rc.2 适配记录](docs/dsh-0.1.5-rc.2-adaptation.md)列明该版本的验证结果。已发布版本的验证证据在下方单独列明。
